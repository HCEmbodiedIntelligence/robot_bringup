...
此后重新运行同一命令仍然只打开网页，不会自动开启上次的机器人。点击“开启机器人”才按已保存配置启动厂商驱动及平台服务；命令行的遥操作/相机选项只预选网页开关。
...
- 同一插件目录只允许一个管理入口和一套机器人运行进程；更换网页端口或状态目录也不能重复启动。
  锁由操作系统在退出时释放，无需删除锁文件。Linux 下入口或网页进程异常退出时，会通知其受控子进程正常关闭。
